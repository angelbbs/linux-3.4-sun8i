...
build_gpu_sun8iw3()
{
	...
	unset LANGUAGE
	make -j2 -C modules/mali LICHEE_MOD_DIR=${LICHEE_MOD_DIR} LICHEE_KDIR=${LICHEE_KDIR} \
	install
}
...
build_gpu_sun9iw1()
{
	if [ "x${LICHEE_PLATFORM}" = "xandroid" ] ; then
		unset OUT
		unset TOP
		make -j2 -C modules/rogue_km/build/linux/sunxi_android \
...
build_kernel()
{
	...
	make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} -j2 uImage modules
